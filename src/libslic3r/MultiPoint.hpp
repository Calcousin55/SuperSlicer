...
// note: SquareLengthType is int64 becasue it's not like it won't be enough, we're looking at deviation, not path length.
template<typename InputIterator, typename OutputIterator, typename PointGetter>
inline OutputIterator douglas_peucker_impl_new(InputIterator begin, InputIterator end, OutputIterator out, const coord_t tolerance, PointGetter point_getter)
{
    using InputIteratorCategory = typename std::iterator_traits<InputIterator>::iterator_category;
    static_assert(std::is_base_of_v<std::input_iterator_tag, InputIteratorCategory>);
    if (begin != end) {
...
inline OutputIterator douglas_peucker_impl(
    ...
    for(InputIterator it = begin; it != end; ++it)
        v.push_back(point_getter(*it));
    Polyline v1_out;
    //douglas_peucker_impl_new(v.begin(), v.end(), std::back_inserter(v1_out.points), tolerance, [](const Point &p) { return p; });
    douglas_peucker_double(v.begin(), v.end(), std::back_inserter(v1_out.points), tolerance, [](const Point &p) { return p; });
    Polyline v2_out;
    douglas_peucker_old<int64_t>(v.begin(), v.end(), std::back_inserter(v2_out.points), tolerance, [](const Point &p) { return p; });
    if (v1_out.points != v2_out.points) {
        std::cout<<"v1_out="<<v1_out.to_debug_string()<<"\n";
        std::cout<<"v2_out="<<v2_out.to_debug_string()<<"\n";
    }
    //assert(v1_out.points == v2_out.points);
    if (v1_out.size() > 2) {
        v1_out.assert_valid();
    }
#endif
