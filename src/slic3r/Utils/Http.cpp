...
struct form_file
{
    ...
    form_file(fs::path const& p, const boost::filesystem::ifstream::off_type offset, const size_t content_length)
        : ifs(p.string(), std::ios::in | std::ios::binary), init_offset(offset), content_length(content_length)
    {}
};
...
void Http::priv::form_add_file(const char *name, const fs::path &path, const char* filename, boost::filesystem::ifstream::off_type offset, size_t length)
{
    ...
    auto &form_file = form_files.back();
    size_t size = length;
    if (length == 0) {
        form_file.ifs.seekg(0, std::ios::end);
        size = form_file.ifs.tellg() - offset;
    }
    form_file.ifs.seekg(offset);
	...
	if (filename != nullptr) {
		::curl_formadd(&form, &form_end,
			...
			CURLFORM_STREAM, static_cast<void*>(&form_file),
